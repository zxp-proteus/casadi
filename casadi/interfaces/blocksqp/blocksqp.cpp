...
namespace casadi {
  ...
  void Blocksqp::init(const Dict& opts) {
    ...
    if (!block_hess_) {
      ...
      while (ind < nx_) {
        ...
        while (ind<next && ind<nx_) {
          for (s_t k=colind[ind]; k<colind[ind+1]; ++k) next = max(next, 1+row[k]);
          ind++;
        }
    ...
    s_t n_hess = hess_update_==1 || hess_update_==4 ? 2 : 1;
  ...
  s_t Blocksqp::run(BlocksqpMemory* m, s_t maxIt, s_t warmStart) const {
    s_t it, infoQP = 0;
  ...
  calcLagrangeGradient(BlocksqpMemory* m,
    ...
    // - lambdaT * constrJac
    const s_t* jacIndRow = Asp_.row();
    const s_t* jacIndCol = Asp_.colind();
    for (s_t iVar=0; iVar<nx_; iVar++) {
      for (s_t iCon=jacIndCol[iVar]; iCon<jacIndCol[iVar+1]; iCon++) {
   ...
   */
  void Blocksqp::
  calcLagrangeGradient(BlocksqpMemory* m, double* grad_lag, s_t flag) const {
   ...
   * lambda = lambdaQP
   */
  s_t Blocksqp::fullstep(BlocksqpMemory* m) const {
    double alpha;
    ...
    for (s_t k=0; k<10; k++) {
      ...
      s_t info = evaluate(m, m->trial_xk, &objTrial, m->gk);
      m->nFunCalls++;
   ...
   *
   */
  s_t Blocksqp::filterLineSearch(BlocksqpMemory* m) const {
    double alpha = 1.0;
  ...
  secondOrderCorrection(BlocksqpMemory* m, double cNorm, double cNormTrial,
    ...
    for (s_t k=0; k<max_soc_iter_; k++) {
      ...
      s_t info = solveQP(m, get_ptr(deltaXiSOC), get_ptr(lambdaQPSOC), false);
   ...
   */
  s_t Blocksqp::feasibilityRestorationPhase(BlocksqpMemory* m) const {
   ...
   */
  s_t Blocksqp::feasibilityRestorationHeuristic(BlocksqpMemory* m) const {
    m->nRestHeurCalls++;
    ...
    for (s_t k=0; k<nx_; k++) // input: last successful step
      m->trial_xk[k] = m->xk[k];
   ...
   */
  s_t Blocksqp::kktErrorReduction(BlocksqpMemory* m) const {
    s_t info = 0;
   ...
   */
  void Blocksqp::calcInitialHessian(BlocksqpMemory* m, s_t b) const {
    s_t dim = dim_[b];
  ...
  void Blocksqp::resetHessian(BlocksqpMemory* m, s_t b) const {
    s_t dim = dim_[b];
   ...
   */
  void Blocksqp::
  calcHessianUpdate(BlocksqpMemory* m, s_t updateType, s_t hessScaling) const {
    s_t nBlocks;
    bool firstIter;
  ...
  void Blocksqp::
  calcHessianUpdateLimitedMemory(BlocksqpMemory* m, s_t updateType, s_t hessScaling) const {
    s_t nBlocks;
    s_t m2, pos, posOldest, posNewest;
    s_t hessDamped, hessSkipped;
  ...
  void Blocksqp::
  computeNextHessian(BlocksqpMemory* m, s_t idx, s_t maxQP) const {
  ...
  solveQP(BlocksqpMemory* m, double* deltaXi, double* lambdaQP,
    ...
    if (!hess_lim_mem_ && maxQP > 2 && matricesChanged) {
        double mu = 1.0 / l;
        double mu1 = 1.0 - mu;
        s_t nBlocks = (which_second_derv_ == 1) ? nblocks_-1 : nblocks_;
        for (s_t b=0; b<nBlocks; b++) {
          s_t dim = dim_[b];
          for (s_t i=0; i<dim; i++) {
            for (s_t j=0; j<dim; j++) {
  ...
  convertHessian(BlocksqpMemory* m) const {
    s_t count, colCountTotal, rowOffset;
    s_t nnz;
