...
namespace casadi {
  ...
  bool BonminUserClass::eval_jac_g(Index n, const Number* x, bool new_x,
    ...
    if (values) {
      ...
      s_t ncol = solver_.jacg_sp_.size2();
      const s_t* colind = solver_.jacg_sp_.colind();
      const s_t* row = solver_.jacg_sp_.row();
  ...
  bool BonminUserClass::eval_h(Index n, const Number* x, bool new_x,
    ...
    if (values) {
      ...
      // Get the sparsity pattern
      s_t ncol = solver_.hesslag_sp_.size2();
      const s_t* colind = solver_.hesslag_sp_.colind();
      const s_t* row = solver_.hesslag_sp_.row();
 ...
 bool BonminUserClass::get_variables_linearity(Index n, Ipopt::TNLP::LinearityType* var_types) {
   for (s_t i=0; i<n; ++i) var_types[i] = Ipopt::TNLP::NON_LINEAR;
 ...
 bool BonminUserClass::get_constraints_linearity(Index m, Ipopt::TNLP::LinearityType* const_types) {
   for (s_t i=0; i<m; ++i) const_types[i] = Ipopt::TNLP::NON_LINEAR;
   // Ipopt::TNLP::NON_LINEAR / Ipopt::TNLP::LINEAR
   return true;
 }
