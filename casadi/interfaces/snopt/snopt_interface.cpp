...
namespace casadi {
  ...
  void SnoptInterface::init(const Dict& opts) {
    ...
    std::vector<s_t> ii = mapping_gradF.sparsity().get_col();
    for (s_t j = 0; j < nnObj_; ++j) {
  ...
  std::string SnoptInterface::formatStatus(s_t status) const {
  ...
  void SnoptInterface::solve(void* mem) const {
    ...
    *res++ = m->jac_fk;
    ...
    for (s_t k = 0; k < A_structure_.nnz(); ++k) {
      s_t i = A_structure_.nonzeros()[k];
      if (i == 0) {
        m->A_data[k] = 0;
      } else if (i > 0) {
    ...
    for (auto&& op : opts_) {
      ...
      if (op.second.can_cast_to(OT_INT)) {
        ...
        s_t flag = setIntParameter(&prob, const_cast<char*>(opname.c_str()),
                                   op.second.to_int());
        if (flag==0) continue;
      }
      ...
      if (op.second.can_cast_to(OT_DOUBLE)) {
        casadi_assert_dev(opname.size() <= 55);
        s_t flag = setRealParameter(&prob, const_cast<char*>(opname.c_str()),
                                    op.second.to_double());
        if (flag==0) continue;
      }
      ...
      if (op.second.can_cast_to(OT_STRING)) {
        ...
        s_t flag = setParameter(&prob, const_cast<char*>(buffer.c_str()));
        if (flag==0) continue;
      }
    ...
    // Run SNOPT
    s_t info = solveC(&prob, Cold_, &m->fk);
  ...
  void SnoptInterface::
  userfun(SnoptMemory* m, s_t* mode, s_t nnObj, s_t nnCon, s_t nnJac, s_t nnL, s_t neJac,
          double* x, double* fObj, double*gObj, double* fCon, double* gCon,
          s_t nState, char* cu, s_t lencu, s_t* iu, s_t leniu, double* ru,
          s_t lenru) const {
    try {
      ...
      for (s_t k = 0; k < nnObj; ++k) m->xk2[k] = x[k];
      ...
      *res++ = m->jac_fk;
      ...
      for (s_t k = 0; k < nnObj; ++k) {
        s_t el = jac_f_fcn_.sparsity_out(1).colind(k);
      ...
      if (!jac_g_fcn_.is_null()) {
        ...
        *res++ = m->jac_gk;
        ...
        s_t kk = 0;
        for (s_t j = 0; j < nnJac; ++j) {
          for (s_t k = A_structure_.colind(j); k < A_structure_.sparsity().colind(j+1); ++k) {
  ...
  void SnoptInterface::
  userfunPtr(s_t * mode, s_t* nnObj, s_t * nnCon, s_t *nJac,
             s_t *nnL, s_t * neJac, double *x, double *fObj,
             double *gObj, double * fCon, double* gCon,
             s_t* nState, char* cu, s_t* lencu, s_t* iu,
             s_t* leniu, double* ru, s_t *lenru) {
