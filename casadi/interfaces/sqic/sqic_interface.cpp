...
namespace casadi {
  ...
  void SqicInterface::init() {
    ...
    for (s_t i=0;i<indH_.size();++i) indH_[i]+=1;
    for (s_t i=0;i<locH_.size();++i) locH_[i]+=1;
    ...
    // Fortran indices are one-based
    for (s_t i=0;i<indA_.size();++i) indA_[i]+=1;
    for (s_t i=0;i<locA_.size();++i) locA_[i]+=1;
    ...
    s_t nnzA=formatA_.size_out(0);
    s_t nnzH=input(CONIC_H).size();
  ...
  map<s_t, string> SqicInterface::calc_flagmap() {
    map<s_t, string> f;

    return f;
  }

  map<s_t, string> SqicInterface::flagmap = SqicInterface::calc_flagmap();

  void SqicInterface::sqic_error(const string& module, s_t flag) {
    // Find the error
    map<s_t, string>::const_iterator it = flagmap.find(flag);
