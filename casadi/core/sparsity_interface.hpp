  ...
  */
  ...
  public:
    ...
    static std::vector< std::vector< MatType > >
      blocksplit(const MatType& x, const std::vector<s_t>& vert_offset,
                 const std::vector<s_t>& horz_offset);
    static std::vector< std::vector< MatType > >
      blocksplit(const MatType& x, s_t vert_incr, s_t horz_incr);
    static MatType veccat(const std::vector< MatType >& x);
    static MatType vec(const MatType& x);
    static MatType repmat(const MatType& x, s_t n, s_t m=1);
    static std::vector<s_t> offset(const std::vector< MatType > &v, bool vert=true);
    static std::vector< MatType > diagsplit(const MatType& x,
                                            const std::vector<s_t>& output_offset);
    static std::vector< MatType > diagsplit(const MatType& x, s_t incr);
    static std::vector< MatType > diagsplit(const MatType& x, s_t incr1, s_t incr2);
    static MatType mtimes(const std::vector<MatType> &args);
    static std::vector<MatType > horzsplit(const MatType& x, s_t incr);
    static std::vector<MatType > vertsplit(const MatType& x, s_t incr);
    static MatType repmat(const MatType &A, const std::pair<s_t, s_t>& rc) {
...
*/
    ...
    inline friend std::vector<MatType >
      horzsplit(const MatType &x, const std::vector<s_t>& offset) {
     ...
     *   horzcat(horzsplit(x, ...)) = x
     */
    inline friend std::vector<MatType > horzsplit(const MatType& x, s_t incr=1) {
     ...
     */
    inline friend std::vector<MatType >
      vertsplit(const MatType& x, const std::vector<s_t>& offset) {
      return MatType::vertsplit(x, offset);
    }

    /** \brief Helper function, get offsets corresponding to a vector of matrices
     */
    inline friend std::vector<s_t > offset(const std::vector<MatType> &v, bool vert=true) {
     ...
     *
     */
    inline friend std::vector<MatType > vertsplit(const MatType &x, s_t incr=1) {
    ...
    inline friend std::vector< std::vector< MatType > >
      blocksplit(const MatType& x,
                 const std::vector<s_t>& vert_offset,
                 const std::vector<s_t>& horz_offset) {
     ...
     */
    inline friend std::vector< std::vector< MatType > >
      blocksplit(const MatType& x, s_t vert_incr=1, s_t horz_incr=1) {
    ...
    friend std::vector< MatType >
      diagsplit(const MatType& x,
                const std::vector<s_t>& output_offset1,
     ...
     */
    inline friend std::vector< MatType >
      diagsplit(const MatType& x, const std::vector<s_t>& output_offset) {
     ...
     */
    inline friend std::vector< MatType >
      diagsplit(const MatType& x, s_t incr=1) {
     ...
     */
    inline friend std::vector< MatType >
      diagsplit(const MatType& x, s_t incr1, s_t incr2) {
    ...
    */
     ...
     */
    inline friend MatType reshape(const MatType& x, s_t nrow, s_t ncol) {
      return MatType::reshape(x, nrow, ncol);
    }

    /** \brief Returns a reshaped version of the matrix, dimensions as a vector
    */
    inline friend MatType reshape(const MatType& x, std::pair<s_t, s_t> rc) {
    ...
    */
    inline friend s_t sprank(const MatType& x) {
      return MatType::sprank(x);
    }

    /** \brief 0-norm (nonzero count) of a Matrix-matrix product
    */
    inline friend s_t norm_0_mul(const MatType &x, const MatType &y) {
    ...
    */
     ...
     */
    inline friend MatType repmat(const MatType &A, s_t n, s_t m=1) {
      return MatType::repmat(A, n, m);
    }

    /** \brief Repeat matrix A n times vertically and m times horizontally
     */
    inline friend MatType repmat(const MatType &A, const std::pair<s_t, s_t>& rc) {
...
/** \@} */
  ...
  template<typename MatType>
  MatType SparsityInterface<MatType>::repmat(const MatType& x, s_t n, s_t m) {
  ...
  template<typename MatType>
  std::vector< std::vector< MatType > >
  SparsityInterface<MatType>::blocksplit(const MatType& x, s_t vert_incr, s_t horz_incr) {
    casadi_assert_dev(horz_incr>=1);
    casadi_assert_dev(vert_incr>=1);
    s_t sz1 = x.size1();
    std::vector<s_t> offset1 = range(0, sz1, vert_incr);
    offset1.push_back(sz1);
    s_t sz2 = x.size2();
    std::vector<s_t> offset2 = range(0, sz2, horz_incr);
  ...
  template<typename MatType>
  std::vector< MatType >
  SparsityInterface<MatType>::diagsplit(const MatType& x, const std::vector<s_t>& output_offset) {
  ...
  template<typename MatType>
  std::vector< MatType >
  SparsityInterface<MatType>::diagsplit(const MatType& x, s_t incr) {
    casadi_assert_dev(incr>=1);
    casadi_assert(x.is_square(), "diagsplit(x,incr)::input must be square but got "
                          + x.dim()  + ".");
    std::vector<s_t> offset2 = range(0, x.size2(), incr);
  ...
  template<typename MatType>
  std::vector< MatType >
  SparsityInterface<MatType>::diagsplit(const MatType& x, s_t incr1, s_t incr2) {
    casadi_assert_dev(incr1>=1);
    casadi_assert_dev(incr2>=1);
    std::vector<s_t> offset1 = range(0, x.size1(), incr1);
    offset1.push_back(x.size1());
    std::vector<s_t> offset2 = range(0, x.size2(), incr2);
    offset2.push_back(x.size2());
  ...
  MatType SparsityInterface<MatType>::mtimes(const std::vector<MatType> &args) {
    ...
    MatType ret = args[0];
    for (s_t i=1; i<args.size(); ++i) ret = MatType::mtimes(ret, args[i]);
    return ret;
  }

  template<typename MatType>
  std::vector<MatType > SparsityInterface<MatType>::horzsplit(const MatType& x, s_t incr) {
    casadi_assert_dev(incr>=1);
    s_t sz2 = x.size2();
    std::vector<s_t> offset2 = range(0, sz2, incr);
    offset2.push_back(sz2);
    return MatType::horzsplit(x, offset2);
  }

  template<typename MatType>
  std::vector<MatType > SparsityInterface<MatType>::vertsplit(const MatType& x, s_t incr) {
    casadi_assert_dev(incr>=1);
    s_t sz1 = x.size1();
    std::vector<s_t> offset1 = range(0, sz1, incr);
    offset1.push_back(sz1);
