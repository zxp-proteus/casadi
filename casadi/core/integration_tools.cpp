...
namespace casadi {
  ...
  std::vector<RealT> collocation_pointsGen(s_t order, const std::string& scheme) {
    if (scheme=="radau") {
  ...
  std::vector<double> collocation_points(s_t order, const std::string& scheme) {
    return collocation_pointsGen<double>(order, scheme);
  }

  std::vector<long double> collocation_pointsL(s_t order, const std::string& scheme) {
    return collocation_pointsGen<long double>(order, scheme);
  }

  Function simpleRK(Function f, s_t N, s_t order) {
    // Consistency check
    casadi_assert(N>=1,
  ...
  void collocation_interpolators(const std::vector<double> & tau_root,
    ...
    for (s_t j=0; j<deg+1; ++j) {
      // Construct Lagrange polynomials to get the polynomial basis at the collocation point
      SX L = 1;
      for (s_t j2=0; j2<deg+1; ++j2) {
        if (j2 != j) {
  ...
  Function simpleIRK(Function f, s_t N, s_t order, const std::string& scheme,
    ...
    for (s_t j=1; j<order+1; ++j) {
      // Expression for the state derivative at the collocation point
      MX xp_j = 0;
      for (s_t r=0; r<=order; ++r) xp_j+= C[j][r]*x[r];
  ...
  Function simpleIntegrator(Function f, const std::string& plugin,
    ...
    // Normalized xdot
    s_t u_offset[] = {0, 1, 1+p_sp.size1()};
    vector<MX> pp = vertsplit(u, vector<s_t>(u_offset, u_offset+3));
    MX h = pp[0];
