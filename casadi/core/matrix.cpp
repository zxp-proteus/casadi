...
namespace casadi {
  ...
  void Matrix<Scalar>::set_precision(s_t precision) { stream_precision_ = precision; }

  template<typename Scalar>
  void Matrix<Scalar>::set_width(s_t width) { stream_width_ = width; }

  template<typename Scalar>
  void Matrix<Scalar>::set_scientific(bool scientific) { stream_scientific_ = scientific; }

  template<typename Scalar>
  s_t Matrix<Scalar>::stream_precision_ = 6;
  template<typename Scalar>
  s_t Matrix<Scalar>::stream_width_ = 0;
  template<typename Scalar>
  ...
  template<typename Scalar>
  void Matrix<Scalar>::rng(s_t seed) {
    rng_.seed(seed);
  }
  ...
  void Matrix<Scalar>::get(Matrix<Scalar>& m, bool ind1,
    ...
    if (rr.is_scalar(size1()) && cc.is_scalar(size2())) {
      s_t k = sparsity().get_nz(rr.scalar(size1()), cc.scalar(size2()));
      if (k>=0) {
        m = nonzeros().at(k);
      } else {
        m = Matrix<Scalar>(1, 1);
  ...
  template<typename Scalar>
  void Matrix<Scalar>::get(Matrix<Scalar>& m, bool ind1,
                                const Slice& rr, const Matrix<s_t>& cc) const {
    // Fall back on IM-IM
  ...
  template<typename Scalar>
  void Matrix<Scalar>::get(Matrix<Scalar>& m, bool ind1,
                                const Matrix<s_t>& rr, const Slice& cc) const {
    // Fall back on IM-IM
  ...
  template<typename Scalar>
  void Matrix<Scalar>::get(Matrix<Scalar>& m, bool ind1,
                             const Matrix<s_t>& rr, const Matrix<s_t>& cc) const {
    // Scalar
    ...
    for (s_t k=0; k<mapping.size(); ++k) m->at(k) = nonzeros().at(mapping[k]);
  }
  ...
  void Matrix<Scalar>::get(Matrix<Scalar>& m, bool ind1, const Slice& rr) const {
    ...
    if (rr.is_scalar(numel())) {
      s_t r = rr.scalar(numel());
      s_t k = sparsity().get_nz(r % size1(), r / size1());
      if (k>=0) {
        m = nonzeros().at(k);
      } else {
        m = Matrix<Scalar>(1, 1);
  ...
  template<typename Scalar>
  void Matrix<Scalar>::get(Matrix<Scalar>& m, bool ind1, const Matrix<s_t>& rr) const {
    // Scalar
  ...
  void Matrix<Scalar>::set(const Matrix<Scalar>& m, bool ind1,
    ...
    if (rr.is_scalar(size1()) && cc.is_scalar(size2()) && m.is_dense()) {
      s_t oldsize = sparsity_.nnz();
      s_t ind = sparsity_.add_nz(rr.scalar(size1()), cc.scalar(size2()));
  ...
  template<typename Scalar>
  void Matrix<Scalar>::set(const Matrix<Scalar>& m, bool ind1,
                             const Slice& rr, const Matrix<s_t>& cc) {
    // Fall back on (IM, IM)
  ...
  template<typename Scalar>
  void Matrix<Scalar>::set(const Matrix<Scalar>& m, bool ind1,
                                const Matrix<s_t>& rr, const Slice& cc) {
    // Fall back on (IM, IM)
  ...
  template<typename Scalar>
  void Matrix<Scalar>::set(const Matrix<Scalar>& m, bool ind1,
                                const Matrix<s_t>& rr, const Matrix<s_t>& cc) {
    // Scalar
    ...
    for (s_t j=0; j<el.size2(); ++j) { // Loop over columns of m
      s_t this_j = cc->at(j) - ind1; // Corresponding column in this
      if (this_j<0) this_j += sz2;
      for (s_t k=el.colind(j); k<el.colind(j+1); ++k) { // Loop over rows of m
        s_t i = m.row(k);
        s_t this_i = rr->at(i) - ind1; // Corresponding row in this
  ...
  void Matrix<Scalar>::set(const Matrix<Scalar>& m, bool ind1, const Slice& rr) {
    ...
    if (rr.is_scalar(numel()) && m.is_dense()) {
      s_t r = rr.scalar(numel());
      s_t oldsize = sparsity_.nnz();
      s_t ind = sparsity_.add_nz(r % size1(), r / size1());
      if (oldsize == sparsity_.nnz()) {
        nonzeros_.at(ind) = m.scalar();
      } else {
        nonzeros_.insert(nonzeros_.begin()+ind, m.scalar());
  ...
  template<typename Scalar>
  void Matrix<Scalar>::set(const Matrix<Scalar>& m, bool ind1, const Matrix<s_t>& rr) {
    // Scalar
    ...
    if (rr.sparsity() != m.sparsity()) {
      if (rr.size() == m.size()) {
        ...
        return set(project(m, sp), ind1, Matrix<s_t>::project(rr, sp));
      } else if (m.is_scalar()) {
        // m scalar means "set all"
        if (m.is_dense()) {
    ...
    // Dimensions of this
    s_t sz1 = size1(), sz2 = size2(), sz = nnz(), nel = numel(), rrsz = rr.nnz();
    ...
    std::vector<s_t> new_row=sparsity().get_row(), new_col=sparsity().get_col(), nz(rr.nonzeros());
    new_row.reserve(sz+rrsz);
    new_col.reserve(sz+rrsz);
    nz.reserve(rrsz);
    for (std::vector<s_t>::iterator i=nz.begin(); i!=nz.end(); ++i) {
      if (ind1) (*i)--;
      if (*i<0) *i += nel;
  ...
  template<typename Scalar>
  void Matrix<Scalar>::get_nz(Matrix<Scalar>& m, bool ind1, const Matrix<s_t>& kk) const {
    // Scalar
    ...
    // Get nonzeros of kk
    const std::vector<s_t>& k = kk.nonzeros();
    s_t sz = nnz();
  ...
  template<typename Scalar>
  void Matrix<Scalar>::set_nz(const Matrix<Scalar>& m, bool ind1, const Matrix<s_t>& kk) {
    // Scalar
    if (kk.is_scalar(true)) {
  ...
  Matrix<Scalar> Matrix<Scalar>::einstein(
    ...
    std::vector<s_t> iter_dims;
    std::vector<s_t> strides_a;
    std::vector<s_t> strides_b;
    std::vector<s_t> strides_c;
    s_t n_iter = einstein_process(A, B, C, dim_a, dim_b, dim_c, a, b, c,
  ...
  void Matrix<Scalar>::print_dense(std::ostream &stream, bool truncate) const {
    ...
    std::vector<s_t> ind(colind, colind+size2+1);
  ...
  template<typename Scalar>
  void Matrix<Scalar>::reserve(s_t nnz) {
    reserve(nnz, size2());
  }

  template<typename Scalar>
  void Matrix<Scalar>::reserve(s_t nnz, s_t ncol) {
    nonzeros().reserve(nnz);
  }

  template<typename Scalar>
  void Matrix<Scalar>::resize(s_t nrow, s_t ncol) {
  ...
  Matrix<Scalar>::Matrix(const std::vector< std::vector<double> >& d) {
    // Get dimensions
    s_t nrow=d.size();
    s_t ncol=d.empty() ? 1 : d.front().size();
  ...
  template<typename Scalar>
  Matrix<Scalar>::Matrix(s_t nrow, s_t ncol) : sparsity_(nrow, ncol) {
  }

  template<typename Scalar>
  Matrix<Scalar>::Matrix(const std::pair<s_t, s_t>& rc) : sparsity_(rc) {
  }
  ...
  template<typename Scalar>
  void Matrix<Scalar>::erase(const std::vector<s_t>& rr, const std::vector<s_t>& cc, bool ind1) {
    // Erase from sparsity pattern
    std::vector<s_t> mapping = sparsity_.erase(rr, cc, ind1);

    // Update non-zero entries
    for (s_t k=0; k<mapping.size(); ++k)
  ...
  template<typename Scalar>
  void Matrix<Scalar>::erase(const std::vector<s_t>& rr, bool ind1) {
    // Erase from sparsity pattern
    std::vector<s_t> mapping = sparsity_.erase(rr, ind1);
  ...
  template<typename Scalar>
  void Matrix<Scalar>::remove(const std::vector<s_t>& rr, const std::vector<s_t>& cc) {
    casadi_assert_bounded(rr, size1());
    casadi_assert_bounded(cc, size2());

    // Remove by performing a complementary slice
    std::vector<s_t> rrc = complement(rr, size1());
    std::vector<s_t> ccc = complement(cc, size2());
  ...
  template<typename Scalar>
  void Matrix<Scalar>::enlarge(s_t nrow, s_t ncol, const std::vector<s_t>& rr,
  ...
  template<typename Scalar>
  Matrix<Scalar> Matrix<Scalar>::triplet(const std::vector<s_t>& row,
  ...
  template<typename Scalar>
  Matrix<Scalar> Matrix<Scalar>::eye(s_t n) {
  ...
  template<typename Scalar>
  Matrix<Scalar> Matrix<Scalar>::inf(const std::pair<s_t, s_t>& rc) {
    return inf(rc.first, rc.second);
  }

  template<typename Scalar>
  Matrix<Scalar> Matrix<Scalar>::inf(s_t nrow, s_t ncol) {
  ...
  template<typename Scalar>
  Matrix<Scalar> Matrix<Scalar>::nan(const std::pair<s_t, s_t>& rc) {
    return nan(rc.first, rc.second);
  }

  template<typename Scalar>
  Matrix<Scalar> Matrix<Scalar>::nan(s_t nrow, s_t ncol) {
  ...
  template<typename Scalar>
  bool Matrix<Scalar>::is_equal(const Matrix<Scalar> &x, const Matrix<Scalar> &y, s_t depth) {
  ...
  template<typename Scalar>
  Matrix<Scalar> Matrix<Scalar>::dep(s_t ch) const {
    casadi_error("'dep' not defined for " + type_name());
  }

  template<typename Scalar>
  s_t Matrix<Scalar>::n_dep() const {
  ...
  template<typename Scalar>
  void Matrix<Scalar>::set_max_depth(s_t eq_depth) {
    casadi_error("'set_max_depth' not defined for " + type_name());
  }

  template<typename Scalar>
  s_t Matrix<Scalar>::get_max_depth() {
  ...
  Matrix<Scalar> Matrix<Scalar>::det(const Matrix<Scalar>& x) {
    ...
    if (min_row <= min_col) {
      ...
      // Expand along col i
      s_t i = col_count.sparsity().row(min_col);

      Matrix<Scalar> col = x(Slice(0, n), i);

      const s_t* row_i = col.row();

      for (s_t k=0; k<col.nnz(); ++k) {
        // Sum up the cofactors
  ...
  Matrix<Scalar> Matrix<Scalar>::minor(const Matrix<Scalar>& x,
    ...
    std::vector<s_t> col = x.sparsity().get_col();
    const s_t* row = x.sparsity().row();

    for (s_t k=0; k<x.nnz(); ++k) {
      s_t i1 = col[k];
      s_t j1 = row[k];
  ...
  template<typename Scalar>
  Matrix<Scalar> Matrix<Scalar>::cofactor(const Matrix<Scalar>& A, s_t i, s_t j) {

    // Calculate the i, j minor
    Matrix<Scalar> minor_ij = minor(A, i, j);
    // Calculate the cofactor
    s_t sign_i = 1-2*((i+j) % 2);
  ...
  template<typename Scalar>
  Matrix<Scalar> Matrix<Scalar>::reshape(const Matrix<Scalar>& x, s_t nrow, s_t ncol) {
  ...
  Matrix<Scalar> Matrix<Scalar>::trace(const Matrix<Scalar>& x) {
    ...
    s_t size2 = x.size2();
    const s_t *colind=x.colind(), *row=x.row();
    for (s_t c=0; c<size2; c++) {
  ...
  Matrix<Scalar> Matrix<Scalar>::horzcat(const std::vector<Matrix<Scalar> > &v) {
    ...
    for (s_t i=0; i<v.size(); ++i) sp[i] = v[i].sparsity();
  ...
  Matrix<Scalar>::horzsplit(const Matrix<Scalar>& x, const std::vector<s_t>& offset) {
  ...
  Matrix<Scalar> Matrix<Scalar>::vertcat(const std::vector<Matrix<Scalar> > &v) {
    ...
    for (s_t i=0; i<v.size(); ++i) vT[i] = v[i].T();
    return horzcat(vT).T();
  }

  template<typename Scalar>
  std::vector< Matrix<Scalar> >
  Matrix<Scalar>::vertsplit(const Matrix<Scalar>& x, const std::vector<s_t>& offset) {
  ...
  template<typename Scalar>
  std::vector< Matrix<Scalar> >
  Matrix<Scalar>::diagsplit(const Matrix<Scalar>& x, const std::vector<s_t>& offset1,
  ...
  qr_sparse(const Matrix<Scalar>& A, Matrix<Scalar>& V, Matrix<Scalar> &R,
            Matrix<Scalar>& beta, std::vector<s_t>& pinv) {
    // Calculate the pattern
    Sparsity spV, spR;
    vector<s_t> leftmost, parent;
    A.sparsity().qr_sparse(spV, spR, pinv, leftmost, parent);
    // Calculate the nonzeros
    s_t nrow_ext = spV.size1(), ncol = spV.size2();
    V = nan(spV);
    R = nan(spR);
    beta = nan(ncol, 1);
    vector<s_t> iw(nrow_ext + ncol);
  ...
  qr_solve(const Matrix<Scalar>& b, const Matrix<Scalar>& v,
           ...
           const std::vector<s_t>& pinv, bool tr) {
    // Get dimensions, check consistency
    s_t ncol = v.size2();
    s_t nrow = b.size1(), nrhs = b.size2();
  ...
  Matrix<Scalar> Matrix<Scalar>::solve(const Matrix<Scalar>& a, const Matrix<Scalar>& b) {
    ...
    if (a.is_tril()) {
      ...
      Matrix<Scalar> x = b;
      const s_t*  Arow = a.row();
      const s_t*  Acolind = a.colind();
      const std::vector<Scalar> & Adata = a.nonzeros();
      for (s_t i=0; i<a.size2(); ++i) { // loop over columns forwards
        for (s_t k=0; k<b.size2(); ++k) { // for every right hand side
          if (!x.has_nz(i, k)) continue;
          x(i, k) /= a(i, i);
          for (s_t kk=Acolind[i+1]-1; kk>=Acolind[i] && Arow[kk]>i; --kk) {
            s_t j = Arow[kk];
      ...
      const s_t*  Acolind = a.colind();
      const std::vector<Scalar> & Adata = a.nonzeros();
      for (s_t i=a.size2()-1; i>=0; --i) { // loop over columns backwards
        for (s_t k=0; k<b.size2(); ++k) { // for every right hand side
          if (!x.has_nz(i, k)) continue;
          x(i, k) /= a(i, i);
          for (s_t kk=Acolind[i]; kk<Acolind[i+1] && Arow[kk]<i; ++kk) {
            s_t j = Arow[kk];
            x(j, k) -= Adata[kk]*x(i, k);
          }
        }
      ...
      std::vector<s_t> inv_colperm(colperm.size());
      for (s_t k=0; k<colperm.size(); ++k)
  ...
  Matrix<Scalar> Matrix<Scalar>::kron(const Matrix<Scalar>& a, const Matrix<Scalar>& b) {
    ...
    for (s_t a_cc=0; a_cc<a_ncol; ++a_cc) {
      ...
      for (s_t b_cc=0; b_cc<b_ncol; ++b_cc) {
        ...
        for (s_t a_el=a_colind[a_cc]; a_el<a_colind[a_cc+1]; ++a_el) {
          Scalar a_v = v_a[a_el];
          // Loop over existing nonzeros
          for (s_t b_el=b_colind[b_cc]; b_el<b_colind[b_cc+1]; ++b_el) {
            Scalar b_v = v_b[b_el];
            ret_ptr[k++] = a_v*b_v;
          }
        }
  ...
  Matrix<Scalar> Matrix<Scalar>::diag(const Matrix<Scalar>& A) {
    ...
    for (s_t k=0; k<mapping.size(); k++) ret.nz(k) = A.nz(mapping[k]);
    return ret;
  }
  ...
  Matrix<Scalar> Matrix<Scalar>::norm_inf_mul(const Matrix<Scalar>& x,
    ...
    std::vector<s_t> iwork(x.size1()+1+y.size2());
  ...
  gauss_quadrature(const Matrix<Scalar> &f,
                   ...
                   const Matrix<Scalar> &b, s_t order) {
  ...
  Matrix<Scalar> Matrix<Scalar>::taylor(const Matrix<Scalar>& f,
                                            const Matrix<Scalar>& x,
                                            const Matrix<Scalar>& a, s_t order) {
  ...
  template<typename Scalar>
  s_t Matrix<Scalar>::n_nodes(const Matrix<Scalar>& x) {
  ...
  Matrix<Scalar> Matrix<Scalar>::sparsify(const Matrix<Scalar>& x, double tol) {
    ...
    std::vector<s_t> new_colind(1, 0), new_row;
  ...
  template<typename Scalar>
  Matrix<Scalar>::operator s_t() const {
    casadi_assert_dev(is_scalar());
    return static_cast<s_t>(scalar());
  }
  ...
  template<>
  void SX::set_max_depth(s_t eq_depth) {
    SXNode::eq_depth_ = eq_depth;
  }

  template<>
  s_t SX::get_max_depth() {
  ...
  bool SX::is_valid_input() const {
    for (s_t k=0; k<nnz(); ++k) // loop over non-zero elements
    ...
    while (!to_be_expanded.empty()) { // as long as there are nodes to be expanded
      ...
      if (to_be_expanded.top()->is_constant()) { // constant nodes are seen as multiples of one
        ...
        if (node->op() == OP_ADD || node->op() == OP_SUB ||
          ...
          s_t ind1 = indices[node->dep(0).get()];
          s_t ind2 = indices[node->dep(1).get()];
          ...
          if (node->op() == OP_MUL) {
            ...
            for (s_t i=0; i<w.size(); ++i) w[i] *= fac;
            ...
            if (node->op() == OP_ADD) {          // Addition: join both sums
              ...
              w = weights[ind1];
              w.reserve(f.size());
              for (s_t i=0; i<weights[ind2].size(); ++i) w.push_back(-weights[ind2][i]);
            }
            ...
            map<SXNode*, s_t> f_ind; // index in f_new

            for (s_t i=0; i<w.size(); i++) {
              // Try to locate the node
              map<SXNode*, s_t>::iterator it = f_ind.find(f[i]);
  ...
  template<>
  SX SX::gauss_quadrature(const SX& f, const SX& x, const SX& a, const SX& b, s_t order,
                          const SX& w) {
      ...
      default:
        {
          ...
          const s_t depth = 2; // NOTE: a higher depth could possibly give more savings
  ...
  template<>
  std::vector<bool> SX::which_depends(const SX &expr, const SX &var, s_t order, bool tr) {
    return _which_depends(expr, var, order, tr);
  }

  template<>
  SX SX::taylor(const SX& f, const SX& x,
                const SX& a, s_t order) {
  ...
  template<>
  SX SX::mtaylor(const SX& f, const SX& x, const SX& a, s_t order) {
    return mtaylor(f, x, a, order, vector<s_t>(x.nnz(), 1));
  }

  SX mtaylor_recursive(const SX& ex, const SX& x, const SX& a, s_t order,
                       const vector<s_t>&order_contributions,
                       const SXElem & current_dx=casadi_limits<SXElem>::one,
                       double current_denom=1, s_t current_order=1) {
  ...
  SX::print_operator(const SX& X, const vector<string>& args) {
    SXElem x = X.scalar();
    s_t ndeps = casadi_math<double>::ndeps(x.op());
  ...
  void SX::shared(vector<SX >& ex,
    ...
    vector<s_t> usecount(work.size(), 0);
      ...
      default:
        {
          ...
          s_t ind = work2[it->i0].get_temp()-1;
          if (ind>=0) {
  ...
  SX SX::eig_symbolic(const SX& m) {
    ...
    vector<s_t> offset;
    vector<s_t> index;
    s_t nb = m.sparsity().scc(offset, index);
    ...
    for (s_t k=0; k<nb; ++k) {
      vector<s_t> r = range(index.at(k), index.at(k+1));
      // det(lambda*I-m) = 0
  ...
  IM IM::from_info(const Dict& info) {
    Sparsity sp = Sparsity::from_info(info.at("sparsity"));
    std::vector<s_t> data = info.at("data");
    return IM(sp, data);
  }
  ...
  template class casadi_limits<s_t>;
  template class Matrix<double>;
  template class Matrix<s_t>;
