...
namespace casadi {
  ...
  r_t Bilin::eval(const double** arg, double** res, s_t* iw, double* w) const {
    return eval_gen<double>(arg, res, iw, w);
  }

  r_t Bilin::eval_sx(const SXElem** arg, SXElem** res, s_t* iw, SXElem* w) const {
    return eval_gen<SXElem>(arg, res, iw, w);
  }

  template<typename T>
  r_t Bilin::eval_gen(const T** arg, T** res, s_t* iw, T* w) const {
    *res[0] = casadi_bilin(arg[0], dep(0).sparsity(), arg[1], arg[2]);
    return 0;
  }

  r_t Bilin::sp_forward(const bvec_t** arg, bvec_t** res, s_t* iw, bvec_t* w) const {
    /* Return value */
    bvec_t r=0;
  ...
  r_t Bilin::sp_reverse(bvec_t** arg, bvec_t** res, s_t* iw, bvec_t* w) const {
    /* Seed */
