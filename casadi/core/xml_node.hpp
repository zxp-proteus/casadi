...
namespace casadi {
  ...
  public:
    ...
    const XmlNode& operator[](s_t i) const;

    /** \brief  Get a reference to a child by its index */
    XmlNode& operator[](s_t i);
    ...
    static void readString(const std::string& str, s_t& val);
    ...
    void dump(std::ostream &stream, s_t indent=0) const;

    std::map<std::string, std::string>  attributes_;
    std::vector<XmlNode>                children_;
    std::map<std::string, s_t>           child_indices_; // the index of the children
