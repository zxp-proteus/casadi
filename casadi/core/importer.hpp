...
namespace casadi {
  ...
  */
  ...
  public:
    ...
    bool has_meta(const std::string& cmd, s_t ind=-1) const;

    /** \brief Get entry as a text */
    std::string get_meta(const std::string& cmd, s_t ind=-1) const;
    ...
    static inline std::string indexed(const std::string& cmd, s_t ind) {
      std::stringstream ss;
      ss << cmd << "[" << ind << "]";
      return ss.str();
    }
    ...
    /** \brief Convert to a type */
    template<typename T>
    T to(const std::string& cmd, s_t ind=-1) const {
      return text2type<T>(get_meta(cmd, ind));
    }

    /** \brief Get entry as a string */
    std::string meta_string(const std::string& cmd, s_t ind=-1) const {
      return to<std::string>(cmd, ind);
    }

    /** \brief Get entry as a vector */
    template<typename T>
    std::vector<T> meta_vector(const std::string& cmd, s_t ind=-1) const {
      return text2vector<T>(get_meta(cmd, ind));
    }

    /** \brief Get entry as a set */
    template<typename T>
    std::set<T> meta_set(const std::string& cmd, s_t ind=-1) const {
      return text2set<T>(get_meta(cmd, ind));
    }

    /** \brief Get entry as an integer */
    s_t meta_int(const std::string& cmd, s_t ind=-1) const {
      return to<s_t>(cmd, ind);
    }
#endif // SWIG
  };
