...
namespace casadi {
  ...
  Sparsity Switch::get_sparsity_in(s_t i) {
    if (i==0) {
  ...
  Sparsity Switch::get_sparsity_out(s_t i) {
    Sparsity ret;
    for (auto&& fk : f_) {
      if (!fk.is_null()) {
  ...
  r_t Switch::eval(const double** arg, double** res, s_t* iw, double* w, void* mem) const {
    // Get the function to be evaluated
    s_t k = arg[0] ? static_cast<s_t>(*arg[0]) : 0;
  ...
  r_t Switch::eval_sx(const SXElem** arg, SXElem** res, s_t* iw, SXElem* w, void* mem) const {
