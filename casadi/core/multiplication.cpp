...
namespace casadi {
  ...
  r_t Multiplication::eval(const double** arg, double** res, s_t* iw, double* w) const {
  ...
  template<typename T>
  r_t Multiplication::eval_gen(const T** arg, T** res, s_t* iw, T* w) const {
  ...
  generate(CodeGenerator& g,
    ...
    s_t nrow_x = dep(1).size1(), nrow_y = dep(2).size1(), ncol_y = dep(2).size2();
