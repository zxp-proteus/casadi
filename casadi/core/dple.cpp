...
namespace casadi {
  ...
  Sparsity Dple::get_sparsity_in(s_t i) {
  ...
  Sparsity Dple::get_sparsity_out(s_t i) {
  ...
  Function Dple::get_forward(s_t nfwd, const std::string& name,
    ...
    MX Qdot = Vdotf.map("map", "serial", nrhs_, {0, 2}, std::vector<s_t>{})
         .map("map", "serial", nfwd, {0, 1}, std::vector<s_t>{})({A, P, Adot, Vdot})[0];
  ...
  Function Dple::get_reverse(s_t nadj, const std::string& name,
                               const std::vector<std::string>& inames,
                               const std::vector<std::string>& onames,
                               const Dict& opts) const {
    ...
    MX Abar = Abarf.map("map", "serial", nrhs_, std::vector<s_t>{1}, {0}).
                    map("map", "serial", nadj, {0, 1}, std::vector<s_t>{})({P, A_rev, Vbar_rev})[0];
