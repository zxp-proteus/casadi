...
namespace casadi {
  ...
  string External::get_name_in(s_t i) {
    if (get_name_in_) {
  ...
  string External::get_name_out(s_t i) {
    if (get_name_out_) {
      // Use function pointer
  ...
  Sparsity GenericExternal::get_sparsity_in(s_t i) {
    ...
    if (get_sparsity_in_) {
      ...
      return Sparsity::compressed(li_.meta_vector<s_t>(name_ + "_SPARSITY_IN", i));
    } else {
      // Fall back to base class
  ...
  Sparsity GenericExternal::get_sparsity_out(s_t i) {
    // Use sparsity retrieval function, if present
    if (get_sparsity_out_) {
      return Sparsity::compressed(get_sparsity_out_(i));
    } else if (li_.has_meta(name_ + "_SPARSITY_OUT", i)) {
      return Sparsity::compressed(li_.meta_vector<s_t>(name_ + "_SPARSITY_OUT", i));
    } else {
      // Fall back to base class
  ...
  void External::init(const Dict& opts) {
    ...
    s_t sz_arg=0, sz_res=0, sz_iw=0, sz_w=0;
    if (work_) {
      s_t flag = work_(&sz_arg, &sz_res, &sz_iw, &sz_w);
      casadi_assert(flag==0, "External: \"work\" failed");
    } else if (li_.has_meta(name_ + "_WORK")) {
      vector<s_t> v = li_.meta_vector<s_t>(name_ + "_WORK");
    ...
    if (jac_sparsity_fcn) {
      ...
      vector<s_t> sp = li_.meta_vector<s_t>("jac_" + name_ + "_SPARSITY_OUT", 0);
  ...
  ::get_forward(s_t nfwd, const std::string& name,
    ...
    if (n!=nfwd || !has_forward(nfwd)) {
      ...
      return fwd1.map(name, "serial", nfwd, range(n_in_+n_out_), std::vector<s_t>(), opts);
      //casadi_error("Internal error: Refactoring needed, cf. #1055");
    }
    return external(name, li_, opts);
  }

  bool External::has_forward(s_t nfwd) const {
  ...
  ::get_reverse(s_t nadj, const std::string& name,
    ...
    if (n!=nadj || !has_reverse(nadj)) {
      ...
      return adj1.map(name, "serial", nadj, range(n_in_+n_out_), std::vector<s_t>(), opts);
      //casadi_error("Internal error: Refactoring needed, cf. #1055");
    }
    return external(name, li_, opts);
  }

  bool External::has_reverse(s_t nadj) const {
