...
namespace casadi {
  ...
  MX MXFunction::instruction_MX(s_t k) const {
    return algorithm_.at(k).data;
  }

  std::vector<s_t> MXFunction::instruction_input(s_t k) const {
  ...
  std::vector<s_t> MXFunction::instruction_output(s_t k) const {
    auto e = algorithm_.at(k);
    if (e.op==OP_OUTPUT) {
      const IOInstruction* io = static_cast<const IOInstruction*>(e.data.get());
      return { io->ind() };
  ...
  void MXFunction::init(const Dict& opts) {
    ...
    vector<s_t> place_in_alg;
    place_in_alg.reserve(nodes.size());

    // Input instructions
    vector<pair<s_t, MXNode*> > symb_loc;

    // Count the number of times each node is used
    vector<s_t> refcount(nodes.size(), 0);
    ...
    for (MXNode* n : nodes) {
      ...
      if (op>=0) {
        ...
        s_t oind = n->which_output();

        // Get the index of the parent node
        s_t pind = place_in_alg[n->dep(0)->temp];

        // Save location in the algorithm element corresponding to the parent node
        s_t& otmp = algorithm_[pind].res.at(oind);
        if (otmp<0) {
    ...
    vector<s_t>& place = place_in_alg; // Reuse memory as it is no longer needed
    place.resize(nodes.size());

    // Stack with unused elements in the work vector, sorted by sparsity pattern
    SPARSITY_MAP<s_t, stack<s_t> > unused_all;
    ...
    for (auto&& e : algorithm_) {
      ...
      s_t first_to_free = 0;
      s_t last_to_free = e.data->n_inplace();
      for (s_t task=0; task<2; ++task) {

        // Dereference or free the memory of the arguments
        for (s_t c=last_to_free-1; c>=first_to_free; --c) { // reverse order so that the
          ...
          if (ch_ind>=0) {
            ...
            if (live_variables && remaining==0) {
              ...
              s_t nnz = nodes[ch_ind]->sparsity().nnz();
  ...
  r_t MXFunction::eval(const double** arg, double** res, s_t* iw, double* w, void* mem) const {
  ...
  r_t MXFunction::sp_reverse(bvec_t** arg, bvec_t** res, s_t* iw, bvec_t* w, void* mem) const {
    // Temporaries to hold pointers to operation input and outputs
    ...
    for (auto it=algorithm_.rbegin(); it!=algorithm_.rend(); it++) {
      if (it->op==OP_INPUT) {
        ...
        s_t nnz=it->data.nnz();
        s_t i=it->data->ind();
        s_t nz_offset=it->data->offset();
        bvec_t* argi = arg[i];
        bvec_t* w1 = w + workloc_[it->res.front()];
        if (argi!=0) for (s_t k=0; k<nnz; ++k) argi[nz_offset+k] |= w1[k];
        fill_n(w1, nnz, 0);
      } else if (it->op==OP_OUTPUT) {
        // Pass output seeds
        s_t nnz=it->data.dep().nnz();
        s_t i=it->data->ind();
        s_t nz_offset=it->data->offset();
        bvec_t* resi = res[i] ? res[i] + nz_offset : 0;
        bvec_t* w1 = w + workloc_[it->arg.front()];
        if (resi!=0) {
          for (s_t k=0; k<nnz; ++k) w1[k] |= resi[k];
          fill_n(resi, nnz, 0);

        }
      } else {
        // Point pointers to the data corresponding to the element
        for (s_t i=0; i<it->arg.size(); ++i)
          arg1[i] = it->arg[i]>=0 ? w+workloc_[it->arg[i]] : 0;
        for (s_t i=0; i<it->res.size(); ++i)
  ...
  std::vector<MX> MXFunction::symbolic_output(const std::vector<MX>& arg) const {
    ...
    for (s_t i=0; i<arg.size() && input_given; ++i) {
  ...
  void MXFunction::eval_mx(const MXVector& arg, MXVector& res,
    ...
    for (s_t i=0; i<in_.size(); ++i) arg_split[i] = in_[i].split_primitives(arg[i]);

    // Allocate storage for split outputs
    vector<vector<MX> > res_split(out_.size());
    for (s_t i=0; i<out_.size(); ++i) res_split[i].resize(out_[i].n_primitives());
    ...
    for (auto it=algorithm_.begin(); it!=algorithm_.end(); ++it, ++alg_counter) {
      if (it->op == OP_INPUT) {
        ...
        for (s_t i=0; i<arg1.size(); ++i) {
          s_t el = it->arg[i]; // index of the argument
        ...
        // Get the result
        for (s_t i=0; i<res1.size(); ++i) {
          s_t el = it->res[i]; // index of the output
    ...
    // Join split outputs
    for (s_t i=0; i<res.size(); ++i) res[i] = out_[i].join_primitives(res_split[i]);
  }
  ...
  void MXFunction::ad_forward(const std::vector<std::vector<MX> >& fseed,
    ...
    for (auto&& r : fseed) {
      if (purgable(r)) {
        ...
        vector<s_t> index_purged;
        for (s_t d=0; d<nfwd; ++d) {
          if (purgable(fseed[d])) {
            for (s_t i=0; i<fsens[d].size(); ++i) {
        ...
        // Fetch result
        for (s_t d=0; d<fseed_purged.size(); ++d) {
  ...
  void MXFunction::ad_reverse(const std::vector<std::vector<MX> >& aseed,
    ...
    for (auto&& r : aseed) {
      ...
      if (purgable(r)) {
        ...
        // Fetch result
        for (s_t d=0; d<aseed_purged.size(); ++d) {
  ...
  r_t MXFunction::eval_sx(const SXElem** arg, SXElem** res, s_t* iw, SXElem* w, void* mem) const {
    // Work vector and temporaries to hold pointers to operation input and outputs
    ...
    for (auto&& a : algorithm_) {
      if (a.op==OP_INPUT) {
        ...
        // Point pointers to the data corresponding to the element
        for (s_t i=0; i<a.arg.size(); ++i)
          argp[i] = a.arg[i]>=0 ? w+workloc_[a.arg[i]] : 0;
        for (s_t i=0; i<a.res.size(); ++i)
  ...
  void MXFunction::codegen_body(CodeGenerator& g) const {
    ...
    bool first = true;
    for (s_t i=0; i<workloc_.size()-1; ++i) {
      s_t n=workloc_[i+1]-workloc_[i];
      if (n==0) continue;
      if (first) {
        g << "casadi_real ";
        first = false;
  ...
  void MXFunction::generate_lifted(Function& vdef_fcn, Function& vinit_fcn) const {
    ...
    for (s_t i=0; i<in_.size(); ++i) in_split[i] = in_[i].primitives();

    // Definition of intermediate variables
    vector<MX> y;
    vector<MX> g;
    vector<vector<MX> > f_G(out_.size());
    for (s_t i=0; i<out_.size(); ++i) f_G[i].resize(out_[i].n_primitives());
        ...
        default:
          {
            ...
            for (s_t i=0; i<arg1.size(); ++i) {
              s_t el = e.arg[i]; // index of the argument
            ...
            // Get the result
            for (s_t i=0; i<res1.size(); ++i) {
              s_t el = e.res[i]; // index of the output
              if (el>=0) swork[el] = res1[i];
            }
          }
        }
    ...
    vector<MX> f_out;
    for (s_t i=0; i<out_.size(); ++i) f_out.push_back(out_[i].join_primitives(f_G[i]));
  ...
  const MX MXFunction::mx_in(s_t ind) const {
    return in_.at(ind);
  }
        ...
        case OP_VERTSPLIT:
         ...
         case OP_SETNONZEROS:
            ...
            if (info.find("nz")!=info.end()) {
              ...
              s_t inner_start = inner["start"];
              s_t inner_step  = inner["step"];
              s_t inner_stop  = inner["stop"];
              s_t outer_start = outer["start"];
              s_t outer_step  = outer["step"];
              s_t outer_stop  = outer["stop"];
              ...
              s_t N = range(outer_start, outer_stop, outer_step).size();
              s_t M = range(inner_start, inner_stop, inner_step).size();
