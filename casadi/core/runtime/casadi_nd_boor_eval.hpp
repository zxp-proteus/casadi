...
void casadi_nd_boor_eval(T1* ret, s_t n_dims, const T1* all_knots, const s_t* offset, const s_t* all_degree, const s_t* strides, const T1* c, s_t m, const T1* all_x, const s_t* lookup_mode, s_t reverse, s_t* iw, T1* w) { // NOLINT(whitespace/line_length)
