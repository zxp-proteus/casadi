...
namespace casadi {
  ...
  void DaeBuilder::disp(std::ostream& stream, bool more) const {
    ...
    if (!this->init.empty()) {
      ...
      for (s_t k=0; k<this->init.size(); ++k) {
  ...
  void DaeBuilder::scale_variables() {
    ...
    for (s_t i=0; i<this->x.size(); ++i) this->ode[i] = *it++ / nominal(this->x[i]);
    for (s_t i=0; i<this->s.size(); ++i) this->dae[i] = *it++;
    for (s_t i=0; i<this->z.size(); ++i) this->alg[i] = *it++;
    for (s_t i=0; i<this->q.size(); ++i) this->quad[i] = *it++ / nominal(this->q[i]);
    for (s_t i=0; i<this->d.size(); ++i) this->ddef[i] = *it++ / nominal(this->d[i]);
    for (s_t i=0; i<this->y.size(); ++i) this->ydef[i] = *it++ / nominal(this->y[i]);
    for (s_t i=0; i<this->init.size(); ++i) this->init[i] = *it++;
  ...
  void DaeBuilder::sort_d() {
    ...
    // BLT transformation
    vector<s_t> rowperm, colperm, rowblock, colblock, coarse_rowblock, coarse_colblock;
  ...
  void DaeBuilder::eliminate_d() {
    ...
    // Get the modified expressions
    vector<MX>::const_iterator it=ex.begin();
    for (s_t i=0; i<this->x.size(); ++i) this->ode[i] = *it++;
    for (s_t i=0; i<this->s.size(); ++i) this->dae[i] = *it++;
    for (s_t i=0; i<this->z.size(); ++i) this->alg[i] = *it++;
    for (s_t i=0; i<this->q.size(); ++i) this->quad[i] = *it++;
    for (s_t i=0; i<this->y.size(); ++i) this->ydef[i] = *it++;
    for (s_t i=0; i<this->init.size(); ++i) this->init[i] = *it++;
    casadi_assert_dev(it==ex.end());
  }
  ...
  void DaeBuilder::make_semi_explicit() {
    ...
    // BLT transformation
    vector<s_t> rowperm, colperm, rowblock, colblock, coarse_rowblock, coarse_colblock;
    s_t nb = sp.btf(rowperm, colperm, rowblock, colblock,
  ...
  MX DaeBuilder::add_variable(const std::string& name, s_t n) {
  ...
  MX DaeBuilder::add_x(const std::string& name, s_t n) {
  ...
  MX DaeBuilder::add_q(const std::string& name, s_t n) {
  ...
  std::pair<MX, MX> DaeBuilder::add_s(const std::string& name, s_t n) {
  ...
  MX DaeBuilder::add_z(const std::string& name, s_t n) {
  ...
  MX DaeBuilder::add_p(const std::string& name, s_t n) {
  ...
  MX DaeBuilder::add_u(const std::string& name, s_t n) {
  ...
  MX DaeBuilder::add_aux(const std::string& name, s_t n) {
  ...
  std::string DaeBuilder::name_in() {
    stringstream ss;
    ss << "[";
    for (s_t i=0; i!=DAE_BUILDER_NUM_IN; ++i) {
      if (i!=0) ss << ",";
  ...
  std::string DaeBuilder::name_out() {
    stringstream ss;
    ss << "[";
    for (s_t i=0; i!=DAE_BUILDER_NUM_OUT; ++i) {
      if (i!=0) ss << ",";
  ...
  Function DaeBuilder::create(const std::string& fname,
    ...
    for (s_t i=0; i<s_out_noatt.size(); ++i) {
    ...
    vector<vector<s_t> > wanted(DAE_BUILDER_NUM_OUT, vector<s_t>(DAE_BUILDER_NUM_IN, -1));
    for (s_t i=0; i<s_out_noatt.size(); ++i) {
      if (assigned[i]) continue;
    ...
    for (s_t oind=0; oind!=DAE_BUILDER_NUM_OUT; ++oind) {
      for (s_t iind=0; iind!=DAE_BUILDER_NUM_IN; ++iind) {
        // Skip if not wanted
        ...
        for (s_t oind1=oind+1; oind1!=DAE_BUILDER_NUM_OUT; ++oind1) {
        ...
        for (s_t iind1=iind+1; iind1!=DAE_BUILDER_NUM_IN; ++iind1) {
          // Do we really want _all_ the input/output combinations?
          bool all_wanted = true;
          for (s_t k=0; k<ob.size() && all_wanted; ++k) {
        ...
        for (s_t ki=0; ki<ib.size(); ++ki) {
          for (s_t ko=0; ko<ob.size(); ++ko) {
            s_t& ind=wanted[ob[ko]][ib[ki]];
    ...
    for (std::map<std::string, MX>::const_iterator lin_comb_it=lin_comb_.begin();
      ...
      fill(wanted.begin(), wanted.end(), vector<s_t>(DAE_BUILDER_NUM_IN, -1));
      ...
      for (s_t iind1=0; iind1!=DAE_BUILDER_NUM_IN; ++iind1) {
        for (s_t iind2=0; iind2!=DAE_BUILDER_NUM_IN; ++iind2) {
          // Skip if not wanted
          ...
          for (s_t iind=iind1+1; iind!=DAE_BUILDER_NUM_IN; ++iind) {
          ...
          for (s_t iind=iind2+1; iind!=DAE_BUILDER_NUM_IN; ++iind) {
            // Do we really want _all_ the blocks?
            bool all_wanted = true;
            for (s_t k=0; k<ib1.size() && all_wanted; ++k) {
          ...
          for (s_t i=0; symmetric && i<ib1.size(); ++i) symmetric=ib1[i]==ib2[i];
          ...
          for (s_t k1=0; k1<ib1.size(); ++k1) {
            for (s_t k2=0; k2<ib2.size(); ++k2) {
              s_t& ind=wanted[ib1[k1]][ib2[k2]];
              if (ind>=0) {
