...
namespace casadi {
  ...
  s_t SXNode::to_int() const {
    casadi_error("to_int not defined for " + class_name());
  }

  bool SXNode::is_equal(const SXNode* node, s_t depth) const {
    return false;
  }
  ...
  void SXNode::can_inline(std::map<const SXNode*, s_t>& nodeind) const {
    // Add or mark node in map
    std::map<const SXNode*, s_t>::iterator it=nodeind.find(this);
    if (it==nodeind.end()) {
  ...
  std::string SXNode::print_compact(std::map<const SXNode*, s_t>& nodeind,
