...
namespace casadi {
  ...
  calc_function(OracleMemory* m, const std::string& fcn,
    ...
    if (arg) {
      ...
      for (s_t i=0; i<n_in; ++i) m->arg[i] = *arg++;
    }
    ...
    if (monitored) {
      ...
      for (s_t i=0; i<n_in; ++i) {
        s << " " << i << " (" << f.name_in(i) << "): ";
        if (m->arg[i]) {
          // Print nonzeros
          s << "[";
          for (s_t k=0; k<f.nnz_in(i); ++k) {
            if (k!=0) s << ", ";
            s << m->arg[i][k];
          }
          s << "]\n";
    ...
    if (monitored) {
      ...
      for (s_t i=0; i<n_out; ++i) {
        s << " " << i << " (" << f.name_out(i) << "): ";
        if (m->res[i]) {
          // Print nonzeros
          s << "[";
          for (s_t k=0; k<f.nnz_out(i); ++k) {
            if (k!=0) s << ", ";
            s << m->res[i][k];
          }
          s << "]\n";
  ...
  void OracleFunction::print_fstats(const OracleMemory* m) const {
    ...
    char namefmt[10];
    sprint(namefmt, sizeof(namefmt), "%%%ds ", static_cast<s_t>(name_len));
