...
namespace casadi {
  ...
  void ImporterInternal::read_meta(istream& file, s_t& offset) {
    // Loop over the lines
    std::string line;
    while (getline(file, line)) {
      offset++;
  ...
  read_external(const string& sym, bool inlined, istream& file, s_t& offset) {
    // New entry
    stringstream ss;
  ...
  std::string ImporterInternal::get_meta(const std::string& cmd, s_t ind) const {
    if (ind>=0) return get_meta(indexed(cmd, ind));
    casadi_assert(has_meta(cmd), "No such command: " + cmd);
    return meta_.at(cmd).second;
  }

  bool ImporterInternal::has_meta(const std::string& cmd, s_t ind) const {
