...
namespace casadi {
  ...
  void LinsolInternal::linsol_eval_sx(const SXElem** arg, SXElem** res, s_t* iw, SXElem* w,
                                      void* mem, bool tr, s_t nrhs) const {
    casadi_error("eval_sx not defined for " + class_name());
  }

  r_t LinsolInternal::solve(void* mem, const double* A, double* x, s_t nrhs, bool tr) const {
    casadi_error("'solve' not defined for " + class_name());
  }

#if 0
  s_t LinsolInternal::factorize(void* mem, const double* A) const {
  ...
  s_t LinsolInternal::neig(void* mem, const double* A) const {
    casadi_error("'neig' not defined for " + class_name());
  }

  s_t LinsolInternal::rank(void* mem, const double* A) const {
