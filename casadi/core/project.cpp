...
namespace casadi {
  ...
  template<typename T>
  r_t Project::eval_gen(const T** arg, T** res, int* iw, T* w) const {
    casadi_project(arg[0], dep().sparsity(), res[0], sparsity(), w);
    return 0;
  }

  r_t Project::eval(const double** arg, double** res, int* iw, double* w) const {
    return eval_gen<double>(arg, res, iw, w);
  }

  r_t Project::eval_sx(const SXElem** arg, SXElem** res, int* iw, SXElem* w) const {
  ...
  r_t Project::sp_forward(const bvec_t** arg, bvec_t** res, int* iw, bvec_t* w) const {
    sparsity().set(res[0], arg[0], dep().sparsity());
    return 0;
  }

  r_t Project::sp_reverse(bvec_t** arg, bvec_t** res, int* iw, bvec_t* w) const {
