...
namespace casadi {
  ...
  template<> SX SX::dep(s_t ch) const;
  template<> s_t SX::n_dep() const;
  template<> std::string SX::name() const;
  template<> void SX::set_max_depth(s_t eq_depth);
  template<> s_t SX::get_max_depth();
  ...
  SX::reverse(const std::vector<SX> &ex, const std::vector<SX> &arg,
          const std::vector<std::vector<SX> > &v, const Dict& opts);
  template<> std::vector<bool> SX::which_depends(const SX &expr, const SX &var, s_t order, bool tr);
  template<> SX SX::taylor(const SX& f, const SX& x, const SX& a, s_t order);
  template<> SX SX::mtaylor(const SX& f, const SX& x, const SX& a, s_t order);
  template<> SX SX::mtaylor(const SX& f, const SX& x, const SX& a, s_t order,
                            const std::vector<s_t>& order_contributions);
  template<> s_t SX::n_nodes(const SX& x);
  template<> std::string
  ...
  extern template class Matrix<s_t>;
