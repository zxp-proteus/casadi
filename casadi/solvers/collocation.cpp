...
namespace casadi {
  ...
  extern "C"
  r_t CASADI_INTEGRATOR_COLLOCATION_EXPORT
  ...
  void Collocation::setupFG() {
    ...
    for (s_t j=0; j<deg_+1; ++j) {

      // Construct Lagrange polynomials to get the polynomial basis at the collocation point
      Polynomial p = 1;
      for (s_t r=0; r<deg_+1; ++r) {
        if (r!=j) {
    ...
    vector<s_t> v_offset(1, 0);
    for (s_t d=0; d<deg_; ++d) {
    ...
    // For all collocation points
    for (s_t j=1; j<deg_+1; ++j) {
      //for (s_t j=deg_; j>=1; --j) {
