...
namespace casadi {
  ...
  void Scpgen::printIteration(ScpgenMemory* m, std::ostream &stream) const {
    ...
    // Print variables
    for (vector<s_t>::const_iterator i=print_x_.begin(); i!=print_x_.end(); ++i) {
  ...
  void Scpgen::printIteration(ScpgenMemory* m, std::ostream &stream, s_t iter, double obj,
                              double pr_inf, double du_inf, double rg, s_t ls_trials,
  ...
  void Scpgen::line_search(ScpgenMemory* m, s_t& ls_iter, bool& ls_success) const {
    ...
    double F_sens = 0;
    for (s_t i=0; i<nx_; ++i) F_sens += m->dxk[i] * m->gfk[i];
